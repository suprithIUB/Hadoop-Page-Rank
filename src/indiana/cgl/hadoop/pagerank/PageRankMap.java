...
// value: <sourceUrl PageRank#targetUrls>
	public void map(LongWritable key, Text value, Context context)
		throws IOException, InterruptedException {
			...
			{
				//Divide the rankValue equally among the targets
				// Two things to be emitted for an URL
				// 1) for all outlinks from URL :   <outlink from URL,rank> 
				//rank = rankofURL/totaloutgoinglinks 		
				double rankValuePerUrl = rrd.rankValue/(double)rrd.targetUrlsList.size();
				for(Integer i: rrd.targetUrlsList)
				{
					sb.append("#").append(i);
					context.write(new LongWritable(i), new Text(String.valueOf(rankValuePerUrl)));
				}
				
				//2) <URL,outlinks> ;;outlinks should have the "#" seperator
